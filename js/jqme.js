...
return jQuery;}));;
yii=(function($){var pub={reloadableScripts:[],clickableSelector:'a, button, input[type="submit"], input[type="button"], input[type="reset"], input[type="image"]',changeableSelector:'select, input, textarea',getCsrfParam:function(){return $('meta[name=csrf-param]').prop('content');},getCsrfToken:function(){return $('meta[name=csrf-token]').prop('content');},confirm:function(message){return confirm(message);},allowAction:function($e){var message=$e.data('confirm');return message===undefined||pub.confirm(message);},handleAction:function($e){var method=$e.data('method');if(method===undefined){return true;}
var $form=$e.closest('form');var action=$e.prop('href');var newForm=!$form.length||action;if(newForm){if(!action||!action.match(/(^\/|:\/\/)/)){action=window.location.href;}
$form=$('<form method="'+method+'" action="'+action+'"></form>');var target=$e.prop('target');if(target){$form.attr('target',target);}
...
var oldMethod=$form.prop('method');$form.prop('method',method);$form.trigger('submit');$form.prop('method',oldMethod);if(newForm){$form.remove();}
return false;},getQueryParams:function(url){var pos=url.indexOf('?');if(pos<0){return{};}
...
function initCsrfHandler(){$.ajaxPrefilter(function(options,originalOptions,xhr){if(!options.crossDomain&&pub.getCsrfParam()){xhr.setRequestHeader('X-CSRF-Token',pub.getCsrfToken());}});}
function initDataMethods(){var $document=$(document);$document.on('click.yii',pub.clickableSelector,function(event){var $this=$(this);if(pub.allowAction($this)){return pub.handleAction($this);}else{event.stopImmediatePropagation();return false;}});$document.on('change.yii',pub.changeableSelector,function(event){var $this=$(this);if(pub.allowAction($this)){return pub.handleAction($this);}else{event.stopImmediatePropagation();return false;}});}
...
var url=options.url.charAt(0)==='/'?hostInfo+options.url:options.url;if($.inArray(url,loadedScripts)===-1){loadedScripts.push(url);}else{var found=$.inArray(url,$.map(pub.reloadableScripts,function(script){return script.charAt(0)==='/'?hostInfo+script:script;}))!==-1;if(!found){xhr.abort();}}});}
...
(function($){var textarea,staticOffset;var iLastMousePos=0;var iMin=32;var grip;$.fn.TextAreaResizer=function(options){return this.each(function(){textarea=$(this).addClass('processed'),staticOffset=null;$(this).wrap('<div class="resizable-textarea"><span></span></div>').parent().append($('<div class="grippie"></div>').bind("mousedown",{el:this},startDrag));var grippie=$('div.grippie',$(this).parent())[0];grippie.style.marginRight=(grippie.offsetWidth-$(this)[0].offsetWidth)+'px'})
...
return;}
this.$element.val(storedValue);this.options.onRetrieve(this.$element,storedValue);return;}},detectConflict:function(){var self=this;if(this.$element.is('input[type=checkbox], input[type=radio]')){return false;}
...
this.each(function(){if($(this).is('form')){$(this).find(options.inputs).each(function(){returnValue=bind($(this));});}else if($(this).is(options.inputs)){returnValue=bind($(this));}});return'function'===typeof fn?fn():returnValue;}
$.fn.garlic.Constructor=Garlic;$.fn.garlic.defaults={destroy:true,inputs:'input, textarea, select',events:['DOMAttrModified','textInput','input','change','click','keypress','paste','focus'],domain:false,expires:false,conflictManager:{enabled:false,garlicPriority:true,template:'<span class="garlic-swap"></span>',message:'This is your saved data. Click here to see default one',onConflictDetected:function($item,storedVal){return true;}},getPath:function($item){},onRetrieve:function($item,storedVal){}}
$(window).on('load',function(){$('[data-persist="garlic"]').each(function(){$(this).garlic();})});}(window.jQuery||window.Zepto);;
var box=$("#box");function wordCount(){var count=[];var fullStr=box.val()+" ";var initial_whitespace_rExp=/^[^A-Za-z0-9]+/gi;var left_trimmedStr=fullStr.replace(initial_whitespace_rExp,"");var non_alphanumerics_rExp=rExp=/[^A-Za-z0-9'’-]+/gi;var cleanedStr=left_trimmedStr.replace(non_alphanumerics_rExp," ");var splitString=cleanedStr.split(" ");count['words']=splitString.length-1;count['chars']=box.val().length;count['sentences']=box.val().split(/[.?!](?=\s|\n)/).length;count['paragraphs']=box.val().split(/\n[^\n]/).length;count['avg_sentence_length']=Math.ceil(count['words']/count['sentences']);displayCount(count);displayTextBoxes(count);}
...
{var count=[];count['words']=countWords(box.val(),($.cookie('foreignSupport')==1?2:0));chars=box.val().match(/(?:[^\r\n]|\r(?!\n))/g);count['chars']=(chars!==null?chars.length:0);chars_no_spaces=box.val().match(/\S/g);count['chars_no_spaces']=(chars_no_spaces!==null?chars_no_spaces.length:0);sentences=box.val().match(/[^.!?\s][^.!?]*(?:[.!?](?!['"]?\s|$)[^.!?]*)*[.!?]?['"]?(?=\s|$)/g);count['sentences']=(sentences!==null?sentences.length:0);paragraphs=box.val().match(/(\n\n?|^).*?(?=\n\n?|$)/g);count['paragraphs']=(box.val()!=''?(paragraphs!==null?paragraphs.length:0):0);count['avg_sentence_words']=(box.val()!=''?Math.ceil(count['words']/count['sentences']):0);count['avg_sentence_chars']=(box.val()!=''?Math.ceil(count['chars']/count['sentences']):0);displayCount(count);displayTextBoxes(count);}
function nl2br(str,is_xhtml){var breakTag=(is_xhtml||typeof is_xhtml==='undefined')?'<br />':'<br>';return(str+'').replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g,'$1'+breakTag+'$2');}
function displayTextBoxes(count)
{$("#word_count").text(count['words']);$("#character_count").text(count['chars']);$("#character_count_no_spaces").text(count['chars_no_spaces']);$("#sentence_count").text(count['sentences']);$("#paragraph_count").text(count['paragraphs']);$("#avg_sentence_words").text(count['avg_sentence_words']);$("#avg_sentence_chars").text(count['avg_sentence_chars']);}
function displayCount(count){if(count['words']==1){wordOrWords=" word";}else{wordOrWords=" words";}
if(count['chars']==1){charOrChars=" character";}else{charOrChars=" characters";}
$(".counted").text(" "+count['words']+wordOrWords+"\n"+" "+count['chars']+charOrChars);}
function getTotalWeights(arr){var total=0;$.each(arr,function(){total+=this;});return total;}
function keywordDensity(){var max=1000;stats=$.wordStats;stats.latinOnly=($.cookie('foreignSupport')!=1);if($.cookie('stopWordsCommon')==1)
{stats.stopWords=[];}
stats.computeTopWords(max,$('#box'));totalWeights=getTotalWeights(stats.topWeights);density_list=$("#collapseTwo");density_list.empty();var text='';var percentage;for(i=0;i<stats.topWords.length;i++){if(i==10){break;}
percentage=(100*(stats.topWeights[i]/totalWeights)).toFixed(0);density_list.append('<a class="list-group-item" href="#"><span class="badge"> '+stats.topWeights[i]+' ('+percentage+'%)</span><span class="word">'+stats.topWords[i]+'</span></a>');}
stats.clear();}
$(function(){box.keypress(wordCountInternational).blur(wordCountInternational).focus(wordCountInternational).change(wordCountInternational).keyup(wordCountInternational).keydown(wordCountInternational).load(wordCountInternational);box.keypress(keywordDensity).blur(keywordDensity).focus(keywordDensity).change(keywordDensity).keyup(keywordDensity).keydown(keywordDensity).load(keywordDensity);box.bind('paste',function(e){var el=$(this);setTimeout(function(){var text=$(el).val();keywordDensity();},4);});function getQueryParams(qs){qs=qs.split("+").join(" ");var params={},tokens,re=/[?&]?([^=]+)=([^&]*)/g;while(tokens=re.exec(qs)){params[decodeURIComponent(tokens[1])]=decodeURIComponent(tokens[2]);}
return params;}
var $_GET=getQueryParams(document.location.search);body=$("body");body.on('click','#collapseTwo a',function(e){e.preventDefault();if(!$(this).hasClass('active'))
{box_readonly=$('#box_readonly');box_readonly.show();box_readonly.html(nl2br($("#box").val()));box_readonly.highlight($(this).find('.word').text(),{wordsOnly:false});box_readonly.height($('#box').height()+23);$('#form').hide();$(this).siblings().removeClass('active');$(this).addClass('active');}
else
{$('#box_readonly').hide();$('#form').show();$('#collapseTwo').find('a').removeClass('active');}});body.on('click','#box_readonly',function(){$('#box_readonly').hide();$('#form').show();$('#collapseTwo').find('a').removeClass('active');});$('#options').garlic();$('#form').garlic({onRetrieve:function(elem,retrievedValue){keywordDensity();wordCountInternational();}});$('#hide_job_postings').on('click',function(){$.cookie('hide_job_posting','1',{expires:15});$('#job_postings_box').hide();});blog_entry_alert=$('#blog_entry_alert');blog_entry_alert.bind('closed.bs.alert',function(){$.cookie('blog_entry','hide',{expires:7});});if($.cookie('blog_entry')!='hide')
{blog_entry_alert.show();}
new_version_alert=$('#new_version_alert');new_version_alert.bind('closed.bs.alert',function(){$.cookie('new_version','hide',{expires:15});});if($.cookie('new_version')!='hide')
{new_version_alert.show();}
$('#stopWordsCommon').click(function(){if($(this).prop('checked'))
{checked=1;}
else
{checked=0;}
$.cookie('stopWordsCommon',checked,{expires:365});});$('#foreignSupport').click(function(){if($(this).prop('checked'))
{checked=1;}
else
{checked=0;}
$.cookie('foreignSupport',checked,{expires:365});});$('.link-popover').popover();$('#diffLink').click(function(){$.ajax({type:"POST",url:"rbc.php",data:{text:box.val()},dataType:"json"}).done(function(msg){$('.original').html(box.val());$('.changed').html(msg.text);$("#wrapper tr").prettyTextDiff({cleanup:true});box.val(msg.text);});});if(h=$.cookie('box_height'))
{box.height(h);}
$('#box:not(.processed)').TextAreaResizer({onResize:function(e,height){$.cookie('box_height',height+15,{expires:365});}});$('.collapse').collapse('show');$('#clear').on('click',function(){original=box.val();box.val('');});$('#undo').on('click',function(){box.val(original);});box.focus();var _gaq=_gaq||[];_gaq.push(['_setAccount','UA-145467-20']);_gaq.push(['_trackPageview']);(function(){var ga=document.createElement('script');ga.type='text/javascript';ga.async=true;ga.src=('https:'==document.location.protocol?'https://ssl':'http://www')+'.google-analytics.com/ga.js';var s=document.getElementsByTagName('script')[0];s.parentNode.insertBefore(ga,s);})();(function(){var po=document.createElement('script');po.type='text/javascript';po.async=true;po.src='https://apis.google.com/js/platform.js';var s=document.getElementsByTagName('script')[0];s.parentNode.insertBefore(po,s);})();(function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(d.getElementById(id))return;js=d.createElement(s);js.id=id;js.src="//connect.facebook.net/en_US/all.js#xfbml=1&appId=601160713280345";fjs.parentNode.insertBefore(js,fjs);}(document,'script','facebook-jssdk'));!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?'http':'https';if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+'://platform.twitter.com/widgets.js';fjs.parentNode.insertBefore(js,fjs);}}(document,'script','twitter-wjs');});;
